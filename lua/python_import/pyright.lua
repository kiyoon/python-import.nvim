...
-- Also it works without having the reportUndefinedVariable diagnostic enabled.

-- I want to type define an enum for return statuses, but I don't know how to do it in Lua.
...
local function pyright_server()
  ...
  local function is_unresolved_import_error(diagnostic)
    -- return diagnostic.code == "reportUndefinedVariable" or diagnostic.code == "F821"
    -- NOTE: ruff's F821 is the same as reportUndefinedVariable and thus I turned off pyright's reportUndefinedVariable.
    -- and I don't think it's necessary to check for the diagnostic because people won't always have it enabled.
    -- Let's just leave the check with completion items.
    return true
  end
...
local lsp_completion_handler = function(server, result, unresolved_import, bufnr)
  ...
  if #items == 1 then
    ...
    -- Need to be called within a coroutine.
    local choice_idx = utils.pick_one_with_index(items, "Select import for " .. unresolved_import, format_import)
    ...
    -- end
    --
    -- local choice_idx = vim.fn.inputlist(outputs_to_inputlist)
    -- if choice_idx == 0 then
    --   return false
    -- end

    return resolve_import(items[choice_idx], bufnr)
  end
end
...
local lsp_completion = function(diagnostic)
  ...
  local cb = vim.schedule_wrap(function(_, result)
    -- create another coroutine because it calls vim.ui.select which is async and we want to wait for it.
    coroutine.resume(coroutine.create(function()
      local import_status = lsp_completion_handler(server, result, unresolved_import[1], diagnostic.bufnr)
      coroutine.resume(co, import_status)
    end))
  end)

  vim.lsp.buf_request(diagnostic.bufnr, "textDocument/completion", params, cb)

  local import_status = coroutine.yield()
